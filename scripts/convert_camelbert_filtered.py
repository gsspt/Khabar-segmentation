#!/usr/bin/env python3
"""
CAMeL-BERT post-processing with confidence-based filtering.

Key insight: Not all pred=1 tokens are equally confident. High-confidence
boundaries are likely true isnads, while low-confidence ones are noise.

Strategy:
1. Extract all boundary tokens (pred=1)
2. Filter by confidence threshold to keep only high-confidence boundaries
3. Extract positions and cluster

This filters out low-confidence noise while keeping strong signals.

Usage:
    python scripts/convert_camelbert_filtered.py \
...
def extract_chunk_positions(tokens: List[str], offsets: List[tuple], corpus_len: int) -> Dict[int, int]:
...
def extract_boundaries(
    ...
    """Extract high-confidence boundary positions."""
    SPECIAL_TOKENS = {"[PAD]", "[CLS]", "[SEP]", "[UNK]"}
    char_to_info = {}
...
def main():
    parser = argparse.ArgumentParser(
        description="CAMeL-BERT post-processing with confidence-based filtering"
    )
    ...
    parser.add_argument("--confidence_threshold", type=float, default=0.9, help="Confidence threshold (0-1)")
    ...
    chunk_positions, sep_indices = extract_chunk_positions(tokens, offsets, len(corpus_text))
    logger.info(f"  Chunk positions estimated for {len(chunk_positions)} chunks")

    # Extract boundaries with confidence filtering
    logger.info(f"\nExtracting high-confidence boundaries (threshold >= {args.confidence_threshold})...")
    ...
    output = {
        "metadata": {
            "method": "confidence_filtered_extraction",
            "description": (
                "Confidence-based filtering approach. Extracts boundary tokens with "
                f"confidence >= {args.confidence_threshold}, deduplicates, and clusters."
            ),
